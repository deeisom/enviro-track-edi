        ...
        {items.length === 0 ? (
          ...
          items.map(p => (
            <TableRow key={p.id} className="cursor-pointer hover:bg-accent/50">
              <TableCell>
                <Link to={`/projects/${p.id}`} className="font-mono font-medium text-primary hover:underline">{p.projectNumber}</Link>
              </TableCell>
              <TableCell><Link to={`/projects/${p.id}`} className="hover:underline">{p.name}</Link></TableCell>
    ...
    <div className="space-y-4">
      ...
      <div className="flex gap-3">
        <div className="relative flex-1 max-w-sm">
          ...
          <Input placeholder="Search by number, name, or client..." value={search} onChange={e => setSearch(e.target.value)} className="pl-9" />
        </div>
